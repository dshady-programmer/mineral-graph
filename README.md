...
Seeded from the USGS Mineral Commodity Summaries 2026, using the 2025 estimated column for country level production. Operation level tonnages are plausible allocations rather than USGS figures, which is noted in the seed file.# Mineral Supply Chain Network
...
I chose this domain because I have a First Class B.Tech in Mining Engineering from FUTA. The model reflects how material actually moves through a plant rather than how a diagram says it should.
...
CognoDB implements a subset of Cypher, so I wrote a capability probe (`probe.js`) and ran it before writing the seed script rather than discovering the gaps thirty hours in. It creates its own throwaway labels, asserts on each capability, and deletes everything afterwards.
...
scripts/seed.js       validation and idempotent seeding
data/seed.json         the curated dataset
probe.js              CognoDB capability probe
```
